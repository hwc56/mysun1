 ...
 *
 **/
void  NetConfig::SendRegisterData()
{
...
void  NetConfig::GetIpandNetmask()
{
    QProcess p;
    QString msg;
...
void  NetConfig::GetDevicename()
{
    /*
    QProcess  p;
    p.start("cat /proc/net/dev | awk '{print $1$2$3}'|grep :");
    p.waitForFinished();
    ...
    p.start("cat /proc/net/dev | awk '{print $1$2$3}'|grep :");
    p.waitForFinished();
    QString msg1 = p.readAllStandardOutput();
 ...
 */
/****/
void    NetConfig::doChangResolution()
{
...
void    NetConfig::doChangStyleForGetIP()
{
    //QMessageBox msgBox;
 ...
 *
 **/
void    NetConfig::modifyIpandNetMask()
{
...
*********/
#if 0
void    NetConfig::remote_doChangehostnameSlots(   const QString &newhostname)
{
...
void    NetConfig::remote_doChangenetworkSlots(    const QString &net)
{
    //"ip:*.*.*.*\nnetmask:*.*.*.*\ngateway:*.*.*.*\n"
...
void    NetConfig::remote_doChangeresolutionSlots( const QString &resolution)
{
    qDebug()<<resolution; 
}
#endif
